...
def fetch_random_question() -> MultipleChoiceQuestion:
    ...
    if response.status_code == 200:
        ...
        raise GameException("Failed to fetch question from the API")
...
def check_answer(question: MultipleChoiceQuestion, selected_option: int) -> bool:
    ...
    response = requests.get(f"{API_BASE_URL}/question/{question.question_id}/answer/{selected_option}", timeout=5)
    if response.status_code == 200:
        return response.json()["is_correct"]
    else:
        raise GameException("Failed to check answer with the API")
...
def draw_question(screen, question: MultipleChoiceQuestion, selected_option: int=0, score: int=0) -> None:
    ...
    screen.fill(BLACK)
    if question:
        draw_text(question.question_text, 20, 50, LARGE_FONT, WHITE)
        for i, option in enumerate(question.choices):
            color = GREEN if i == selected_option else WHITE
            draw_text(f"{i + 1}. {option}", 50, 150 + i * 40, FONT, color)
    draw_text(f"Score: {score}", 20, HEIGHT - 50, FONT, BLUE)
    pygame.display.flip()
...
def main():
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_DOWN:
                    selected_option = (selected_option + 1) % len(question.choices)
    ...
    # End screen
    screen.fill(BLACK)
    draw_text("Game Over!", WIDTH // 2 - 100, HEIGHT // 2 - 50, LARGE_FONT, RED)
    draw_text(f"Your Score: {score}", WIDTH // 2 - 100, HEIGHT // 2, FONT, WHITE)
    pygame.display.flip()
